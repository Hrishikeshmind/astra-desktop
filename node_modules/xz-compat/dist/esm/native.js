...
import url from 'url';
import { parseLzma2DictionarySize, parseProperties } from './lzma/types.js';
...
const sizeMismatchError = (expected, actual)=>new Error(`Native decode size mismatch (expected ${expected}, got ${actual})`);
...
function createRawDecoder(lzma, filterId, buildOptions) {
    return (input, properties, unpackSize)=>{
        ...
        const stream = lzma.createStream('rawDecoder', {
            filters
        });
 ...
 */ export function tryLoadNative() {
    ...
    const load = ()=>{
        try {
            const bindings = _require('lzma-native');
